...
This is the file for the spoken digit classification task with NbOx memristor-RC (single channel).
...
def target_signal_gen(
    ...
    all_list = pd.read_csv(r'./Data/Voice/NbOx/inputs/steps_rec.csv', header=None).values
...
def Voice_SRC_exp(
    ...
    # Test setting
    if parallel:
        Ts_set = np.array([np.arange(channels)])
    else:
        Ts_set = np.array([[0]])
        channels = 1
    ...
    if identical:
        ...
        Tr_set = np.array([[1, 2, 2, 0]])
        print('direct transfer')
    else:  # multi-channel TS
        Tr_set = np.array([[1, 2, 2, 0],  # l, h, m, h
                           [2, 1, 2, 1],  # m, l, h, l
                           [3, 1, 1, 0]  # h, l, m, h
                           ])
        #                  [0, 1, 2, 3]
        ...
        for k in range(switches):
            for count in range(period):
                ...
                for parallel_count in range(channels):
                    ...
                    _file = r'.\Data\Voice\NbOx\response\Device {}\{}0{}f{}set{}.csv'.format(
                        device_set[pointer],
                        prefix[pointer],
    ...
    if OUTPUT:
        ...
        with open(r"./Data/Voice/NbOx/outputs/{}_train_con_mat{}.csv".format(mode_name, suffix), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
        ...
        with open(r"./Data/Voice/NbOx/outputs/{}_test_con_mat{}.csv".format(mode_name, suffix), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
        ...
        with open(r"./Data/Voice/NbOx/outputs/{}_both_acc{}.csv".format(mode_name, suffix), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            for row in both_acc:
                writer.writerow(row)
...
def con_mat_plot(filename, Device, num_classification):
    ...
    df = pd.read_csv('./Data/Voice/{}/outputs/'.format(Device)+filename+'.csv', header=None, sep='\n')
    ...
    plt.savefig('./Figure/Voice/{}/'.format(Device)+filename+'.svg', format='svg', dpi=300,
...
if __name__ == '__main__':
    ...
    fig_dir =   './Figure/Voice/NbOx'
    data_dir1 = './Data/Voice/NbOx/inputs'
    data_dir2 = './Data/Voice/NbOx/outputs'
    data_dir3 = './Data/Voice/NbOx/response'
    ...
    # create_Voice_signal_file()  # please disable this function when the voice signal file is generated

    Voice_SRC_exp(Device='NbOx', identical=True, channels=1, num_classification=num_classification)  # ID
    Voice_SRC_exp(Device='NbOx', direct_transfer=True, channels=1, num_classification=num_classification)  # DT
    Voice_SRC_exp(Device='NbOx', direct_transfer=False, channels=1, num_classification=num_classification)  # TS

    con_mat_plot('ID_test_con_mat', Device='NbOx', num_classification=num_classification)
    con_mat_plot('TS_test_con_mat', Device='NbOx', num_classification=num_classification)
    con_mat_plot('DT_test_con_mat', Device='NbOx', num_classification=num_classification)
